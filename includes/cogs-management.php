...
function br_cogs_admin_enqueue_scripts( $hook ) {
	if ( 'business-report_page_br-cogs-management' !== $hook ) {
		return;
	}
	wp_enqueue_script(
		'br-cogs-admin-js',
...
function br_cogs_management_page_html() {
    ...
    <div class="wrap br-wrap">
        ...
        <h2 class="nav-tab-wrapper">
            <a href="?page=br-cogs-management&tab=product_list" class="nav-tab <?php echo $active_tab == 'product_list' ? 'nav-tab-active' : ''; ?>"><?php _e('Product List', 'business-report'); ?></a>
            <a href="?page=br-cogs-management&tab=settings" class="nav-tab <?php echo $active_tab == 'settings' ? 'nav-tab-active' : ''; ?>"><?php _e('Settings', 'business-report'); ?></a>
        </h2>
        <div class="br-page-content">
        <?php
        if ($active_tab == 'settings') {
            br_cogs_settings_tab_html();
        } else {
            br_cogs_product_list_tab_html();
        }
        ?>
        </div>
    </div>
...
function br_cogs_product_list_tab_html() {
    ...
    <form id="br-cogs-list-form" method="post">
        <?php
        // The search box is now outside the form, but this is fine as it uses a GET request.
        // This form is for potential bulk actions in the future.
...
function br_cogs_settings_tab_html(){?><form class="br-settings-form" method="post" action="options.php"><?php settings_fields('br_cogs_settings_group');do_settings_sections('br_cogs_settings_page');submit_button();?></form><?php }

function br_cogs_register_settings(){register_setting('br_cogs_settings_group','br_cogs_settings','br_cogs_settings_sanitize');add_settings_section('br_cogs_general_rules_section',__('Set General Cost','business-report'),'br_cogs_general_rules_section_callback','br_cogs_settings_page');add_settings_field('general_mode',__('General Cost Mode','business-report'),'br_cogs_field_general_mode_html','br_cogs_settings_page','br_cogs_general_rules_section');add_settings_field('general_value',__('Value for Calculation','business-report'),'br_cogs_field_general_value_html','br_cogs_settings_page','br_cogs_general_rules_section');add_settings_section('br_cogs_dynamic_rules_section',__('Set Cost Dynamically by Price Range','business-report'),'br_cogs_dynamic_rules_section_callback','br_cogs_settings_page');add_settings_field('dynamic_rules',__('Conditional Rules','business-report'),'br_cogs_field_dynamic_rules_html','br_cogs_settings_page','br_cogs_dynamic_rules_section');add_settings_section('br_cogs_apply_rules_section',__('Apply Rules to Existing Products','business-report'),'br_cogs_apply_rules_section_callback','br_cogs_settings_page');}
add_action('admin_init','br_cogs_register_settings');
